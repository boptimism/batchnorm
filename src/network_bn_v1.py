...
sys.path.append(expanduser("~") + sep +"Projects")# + "modules")
from helpers.DBUtilsClass import Connection
from random import random

class BNv1:
    def __init__(self,layers,learnrate,batchsize,epochs,num_of_trains,num_of_tests,
                 weights,bias,dbrec=0,stop_at=1.,comment=''):
        ...
        if self.dbrec:
            ...
            rec_runs={'neural_network_type':'rbn',
    ...
    def sgd(self,train_inputs,train_labels,test_inputs,test_labels,
        ...
        for p in np.arange(self.epochs):
            ...
            if self.dbrec:
                ...
                self.con.call(sqlstr.format(epochid, self.train_accu[-1],self.train_cost[-1],
                                            self.test_accu[-1], self.test_cost[-1],tend-tstart))
